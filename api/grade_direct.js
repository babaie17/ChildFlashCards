...
export default async function handler(req) {
  try {
    ...
    const fmt = mimeToFormat(file.type); // 'wav' | 'ogg' | 'webm' | 'mp3'

    // Tight rubric for continuous scoring + Chinese tone handling
    const sys =
`You are a strict but kind pronunciation judge.
...
- pass for anything above or equal to 0.7`;
    ...
    const user = { language, expected };

    // Use the Responses API (supports input_audio content)
    ...
    // Responses API → the JSON is in body.output_text (when response_format=json_object)
    let verdict = {};
    try {
...
async function safeBody(r){ const ct=(r.headers.get('content-type')||'').toLowerCase(); return ct.includes('json')?await r.json():{ message: await r.text() }; }
...
function mimeToFormat(m){ const t=(m||'').toLowerCase(); if (t.includes('wav')) return 'wav'; if (t.includes('ogg')) return 'ogg'; if (t.includes('webm')) return 'webm'; if (t.includes('mp3')) return 'mp3'; return 'wav'; }
